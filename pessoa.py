...
NOMES = ['Pedro','Tiago','Gutinho','Miguel','Jonatan','Gel','Minho','Matuto','Jefferson','Wolf','Bebê','Diburro','Vitinho']
POKEMONS =[
  PokemonFogo('Flarion'),
...
class Pessoa:
  ...
  def mostrar_dinheiro(self):
    print('{} você possui ${} em sua conta'.format(self,self.dinheiro))
  
  def ganhar_dinheiro(self,quantidade):
    self.dinheiro += quantidade
    print('{} você ganhou $ {}'.format(self,quantidade))
...
class Player(Pessoa):
  ...
  def escolher_pokemon(self):
    ...
    if self.pokemons:
      while True:
        escolha = input('{} escolha o seu Pokemon: '.format(self))
        try:
          escolha = int(escolha)
    ...
    if random.random() <= 0.3:
      ...
      if escolha == 's':
        if random.random() >= 0.5:
          ...
          print('{} fugiu!! ficastes na mágoa'.format(pokemon))
      else:
        print('Ok, boa viagem')
    else:
